...
currentMessage = ""
topicName = "ex1-basic"
subscriptionName = "failover-sub-0"
...
def my_listener(consumer, message):
    # process message
    # print(message)
    print("Failover Consumer: key='{}'' message='{}' id='{}' at [{}] [{}] [{}]".format(message.partition_key(), message.data(), message.message_id(), str(round(time.time())), datetime.datetime.now().strftime('%m/%d/%Y %H:%M:%S'), str(message.publish_timestamp())))
